	...
	<header ref="navigationRef" class="sticky top-0 z-50 w-full bg-background text-foreground">
		<Container class="flex items-center justify-between p-4">
			<NuxtLink to="/" class="flex-shrink-0">
				<img :src="lightLogoUrl" alt="Logo" class="h-auto dark:hidden scale-[0.6] md:scale-100 transition-transform" :style="{ width: `${logoWidth}px` }" width="150" height="100" />
				<img
					v-if="darkLogoUrl"
					:src="darkLogoUrl"
					alt="Logo (Dark Mode)"
					class="h-auto hidden dark:block scale-[0.6] md:scale-100 transition-transform"
